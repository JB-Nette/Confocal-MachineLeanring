...
def getFileParser(data_path: str,
    ...
    elif microscope_type.lower() == "triton":
        parser = ParseDirectoryTriton(data_path=data_path,
                                      fovstr_length=fovstr_length,
                                      use_existing_filesdata=use_existing_filesdata)
        
    elif microscope_type.lower() == "confocal":
        parser = ParseDirectoryConfocal(data_path=data_path,
                                      fovstr_length=fovstr_length,
                                      use_existing_filesdata=use_existing_filesdata)

    else:
        raise TypeError("Microscope type not recognised.\n"
                        "Should be 'Dory'/'Nemo' or 'Triton'")
...
class ParseDirectory(object):
    ...
    def __init__(self,
        ...
        roi: int
            region of interest to use (triton only)
        ...
        if self.files_df is None:
            ...
            # "tiff_frame" column : frame of the tiff containing that colour image (Triton only)
    ...
    def roiSubsetOfFilesDF(self,
        ...
        assert "roi" in self.files_df.columns, "dataframe does not have a 'roi' column"

        #assert ptypes.is_numeric_dtype(self.files_df["roi"]), (
        #    f"Dataframe's 'roi' column has datatype {self.files_df['roi'].dtype}.\n"
        #    f"Should be a numeric datatype."
        #)
    ...
    def dfToDict(self,
        ...
        roi:int
            Region of Interest to use (if using Triton microscope)
...
class ParseDirectoryTriton(ParseDirectory):

    def __init__(self,
                 triton_subdir: str = "S10",
                 roi: int = None,
                 fovstr_length: int = 3,
                 **kwargs):
        super(ParseDirectoryTriton, self).__init__(**kwargs)
        # reassign data_path to the subdirectory where triton image data is stored
        self.data_path = os.path.join(self.data_path, triton_subdir)
        self.triton_subdir = triton_subdir
        self.roi = roi
        self.fovstr_length = fovstr_length
    ...
    def parseDirectory(self,
        ...
        # set up regular expression patterns for triton type image subdirectories and filename
            ...
            if match_dir:
                ...
                for file in os.listdir(subdir_path):
                    ...
                    if match_file:
                        ...
                        full_path = os.path.join(self.triton_subdir, subdir, match_file.group(0))
    ...
    def _generateFilesDict(self, fovs_to_process, num_bits, hyb_list, type_list, roi,
        ...
        """
        generate files dictionary from Triton file format
...
class ParseDirectoryConfocal(ParseDirectory):

    def __init__(self,
                 fovstr_length: int = 2,
                 **kwargs) -> None:

        super(ParseDirectoryConfocal, self).__init__(**kwargs)
    ...
    def parseDirectory(self,
        ...
        """
        scans each folder looking for all the .tiff files with correct syntax,
        parses information from the corresponding metadata files and xml files,
        creates a dataframe with all the information sorted properly by imaging type, hyb, and fov
        returns a tuple of (dataframe, number of hybs, number of fovs, number of frames per dax)
        optional: save in the data_tables subfolder as a tsv file
        """
        ...
        # set up regular expression patterns for triton type image subdirectories and filename
        filename_pattern = re.compile(r"([a-zA-Z0-9_-]+)_(\d+)_F(\d+).(tif)")
        metadata_pattern = re.compile(r"([a-zA-Z0-9_-]+)_(\d+).(xml)")
        #info_pattern = re.compile(r"([a-zA-Z0-9_-]+)_(\d+)_(metadata).txt")
        info_pattern = re.compile(r"([a-zA-Z0-9_-]+)_(\d+)_(metadata).txt")
        dims_pattern = re.compile(r"Width=(\d+)", re.IGNORECASE)
        color_pattern = re.compile(r"Wavelength=(\d+)", re.IGNORECASE) 
        ...
        for file in self.file_list:
            print("file before match info", file)

            match_info = re.match(info_pattern, file) 

            if match_info:
                print("match info", file)
                try:
                    info_filepath = os.path.join(self.data_path, str(file))
                    with open(info_filepath , 'r') as info_file:
                        filetext = info_file.read()
                        match_dims = re.search(dims_pattern, filetext)                
                        color_list = re.findall(color_pattern, filetext) #find all color channels
                        for n, i in enumerate(color_list):
                            if i == '700':
                                color_list[n] = "Cy5"
                            if i == '600':
                                color_list[n] = "Cy3"  
        ...
        for file in self.file_list:
            match_metafile = re.match(metadata_pattern, file)      
            match_filename = re.match(filename_pattern, file)
                       
            if match_metafile:
                print("match metafile", file)
                metadata_filepath = os.path.join(self.data_path, match_metafile.group(0))          
                try:
                    metadata_file = ET.parse(metadata_filepath)
                    root = metadata_file.getroot()
                    tag = root.find("STACKS")
                    for child in tag:
                        dic_metadata = child.attrib                
                        for keys in dic_metadata.keys():                    
                            for color_num, color in enumerate(color_list):
                                
                                if keys == "ABS_D":
                                    z_pos = dic_metadata[keys]                                 
                                if keys == "ABS_H":
                                    x_pos = dic_metadata[keys]                                            
                                if keys == "ABS_V":
                                    y_pos = dic_metadata[keys]                            
                                if keys == "IMG_REGEX":
                                    Image_name = dic_metadata[keys]
                                    Image_name = Image_name[:-4] #remove .ims
                                    Fov = [val for val in Image_name.split("_")]
                                    Fov = str(Fov[-1]).replace('F', '')
                                    Fov = Fov.zfill(2)
                                    ...
                                    self.files_df.loc[len(self.files_df)] = (
                                        ...
                                        color,  # e.g. Cy5
                                        int(np.divide(int(match_metafile.group(2)),2)),  # hyb
                                        Fov, # str representaion of FOV
                                        1, dims_info, dims_info,  # pixel dimensions
                                        np.nan, np.nan,  # grid positions (defines FOV)
                                        color_num,  # frame of tiff with the colour
                                        z_pos, y_pos, x_pos,  # positions
                                        np.nan,  # slice index
    ...
    def _generateFilesDict(self, fovs_to_process, num_bits, hyb_list, type_list, roi,
        ...
        # check that roi is None for Dory, since Dory does not support ROIs
        if roi is not None:
            print("Confocal does not support ROIs. Setting 'roi' to None")
            roi = None
...
if __name__ == "__main__":
    ...
    if microscope_type == "Dory":
        ...
        jinyue_params = {}
        jinyue_params["num_bits"] = 2
        #jinyue_params["fovstr_length"] = 2
        jinyue_params["hyb_list"] = list(range(1)) * 2
        # jinyue_params["type_list"] = ["Cy5"] * 8 + ["Alexa594"] * 8
        jinyue_params["type_list"] = ["Cy7", ] * 1 + ["Cy5", ] * 1  # A
        # jinyue_params["type_list"] = ["Cy3", ] * 4 + ["Alexa594", ] * 4 + ["Cy5", ] * 4 + ["IR800", ] * 4  # B
        files_dict = myparser.dfToDict([0, 1],
                                       num_bits=jinyue_params["num_bits"],  # number of bits to go until
                                       hyb_list=jinyue_params["hyb_list"],  # list of hyb numbers for each bit
                                       type_list=jinyue_params["type_list"],  # list of filename types
                                       verbose=True,
                                       )
        ...
        myparser = ParseDirectoryTriton(data_path=data_path,
                                        use_existing_filesdata=False,
                                        )
        print(myparser.files_df)
        myparser.parseDirectory(save=True)
        ...
        triton_params = {}
        triton_params["num_bits"] = 16
        triton_params["hyb_list"] = list(range(4)) * 4
        triton_params["type_list"] = ["Cy7"] * 4 + ["Cy5"] * 4 + ["Alexa594"] * 4 + ["Cy3"] * 4
        print("Hyb list:", triton_params["hyb_list"], "Type list:", triton_params["type_list"])
        files_dict = myparser.dfToDict([(0, 0), (0, 1), (0, 2)],
                                       roi=roi,
                                       num_bits=triton_params["num_bits"],  # number of bits to go until
                                       hyb_list=triton_params["hyb_list"],  # list of hyb numbers for each bit
                                       type_list=triton_params["type_list"],  # list of filename types
                                       verbose=True,
                                       )
        
    elif microscope_type == "confocal":
        myparser = ParseDirectoryConfocal(data_path=data_path,
                                        use_existing_filesdata=False,
                                        )
        print(myparser.files_df)
        myparser.parseDirectory(save=True)
        ...
        jinyue_params = {}
        jinyue_params["num_bits"] = 3
        jinyue_params["hyb_list"] = list(range(1)) * 3
        # jinyue_params["type_list"] = ["Cy5"] * 8 + ["Alexa594"] * 8
        jinyue_params["type_list"] = ["Cy3"] * 1 + ["Cy5"] *1 +["Cy7"] *1# A
        # jinyue_params["type_list"] = ["Cy3", ] * 4 + ["Alexa594", ] * 4 + ["Cy5", ] * 4 + ["IR800", ] * 4  # B
        files_dict = myparser.dfToDict([1,2,3],
                                       num_bits=jinyue_params["num_bits"],  # number of bits to go until
                                       hyb_list=jinyue_params["hyb_list"],  # list of hyb numbers for each bit
                                       type_list=jinyue_params["type_list"],  # list of filename types
                                       verbose=True,
